  ...
  try {
    ...
    const config = {
      headers: {
        ...
        Authorization: `Bearer ${token}`, // 确保这里正确设置了Authorization头
      },
    };

    const { data } = await axios.delete(`http://localhost:4000/api/v1/product/${id}`, config);
...
export const getProductDetails = (id) => async(dispatch)=>{
    try{
        ...
        console.log("Requesting URL:", url); // 这将打印出请求的 URL
...
//         // 将对象转换为 JSON 字符串
//         const response = await axios.put(`http://localhost:4000/api/v1/review`, JSON.stringify(formDataObj), config);
...
//         });
//     } catch (error) {
//         console.error("New Review Error:", error);
